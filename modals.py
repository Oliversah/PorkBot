...
class ModalEnviarPalpite(ui.Modal, title="Enviar Palpite"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 1. Envia a notificação em Embed para o canal de registros (Logs públicos/adm)
        ...
        if canal_registros:
            ...
            texto_footer = f"{interaction.user.display_name} (@{interaction.user.name})"
        ...
        # 2. Resposta efêmera em Embed visível apenas para quem enviou o palpite
        ...
        # Rodapé com foto e nome da pessoa + horário pelo timestamp
        embed_usuario.set_footer(
...
class ModalResultadoBolao(ui.Modal, title="Encerrar Bolão"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Busca usuários que acertaram o placar exato
        vencedores = [
            f"<@{user_id}>" 
        ...
        # Descrição sem título no Embed, com a citação e campo de Vencedor(es)
        descricao = (
...
class ModalTitulo(ui.Modal, title="Altere o título do seu Embed."):
    def __init__(self):
        ...
        self.titulo = ui.TextInput(
            ...
            default=config.embed_builder["titulo"],
            required=True
        )
        self.titulo_url = ui.TextInput(
            label="URL do Título",
            placeholder="Ex: https://seusite.com",
            default=config.embed_builder["titulo_url"],
            required=False
        )
...
class ModalDescricao(ui.Modal, title="Altere a descrição do seu Embed"):
    def __init__(self):
        ...
        self.descricao = ui.TextInput(
            ...
            default=config.embed_builder["descricao"],
            required=True
        )
...
class ModalCor(ui.Modal, title="Altere a cor do seu Embed."):
    def __init__(self):
        ...
        self.cor = ui.TextInput(
            ...
            placeholder="Ex: #FF3B30",
            default=config.embed_builder["cor"],
            required=True
        )
...
class ModalAutor(ui.Modal, title="Altere o autor do seu Embed"):
    def __init__(self):
        super().__init__()
        self.autor_nome = ui.TextInput(
            label="Autor",
            placeholder="Nome do autor",
            default=config.embed_builder["autor_nome"],
            required=False
        )
        self.autor_icon = ui.TextInput(
            label="Imagem do Autor",
            placeholder="URL da imagem",
            default=config.embed_builder["autor_icon"],
            required=False
        )
        self.autor_url = ui.TextInput(
            label="URL do Autor",
            placeholder="Ex: https://seusite.com",
            default=config.embed_builder["autor_url"],
            required=False
        )
...
class ModalConfronto(ui.Modal, title="Configurar Confronto"):
    def __init__(self):
        super().__init__()
        self.time_casa = ui.TextInput(
            label="Time da Casa",
            placeholder="Ex: Flamengo",
            default=config.embed_builder["time_casa"],
            required=True
        )
        self.time_visitante = ui.TextInput(
            label="Time Visitante",
            placeholder="Ex: Palmeiras",
            default=config.embed_builder["time_visitante"],
            required=True
        )
...
class ModalRodape(ui.Modal, title="Altere o rodapé do seu Embed"):
    def __init__(self):
        super().__init__()
        self.rodape_texto = ui.TextInput(
            label="Rodapé",
            placeholder="Texto do rodapé",
            default=config.embed_builder["rodape_texto"],
            required=False
        )
        self.rodape_icon = ui.TextInput(
            label="Imagem do Rodapé",
            placeholder="URL da imagem",
            default=config.embed_builder["rodape_icon"],
            required=False
        )
...
class ModalMidia(ui.Modal, title="Altere as imagens do seu Embed"):
    def __init__(self):
        ...
        self.imagem = ui.TextInput(
            ...
            default=config.embed_builder["imagem_url"],
            required=False
        )
        self.thumbnail = ui.TextInput(
            label="Thumbnail",
            placeholder="URL da thumbnail",
            default=config.embed_builder["thumbnail_url"],
            required=False
        )
